// Config
var S3_BUCKET = 'yongliu-s3-bucket';
var ITEMS_TABLE = 'yongliuMediaSharing';
var IDENTITY_POOL_ID = 'us-east-1:f07a5c39-deed-439a-b92e-cfdfa3431d32';
    ...
    lambda.invoke({
      ...
      if (err) {
        ...
        if (!output.login) {
          ...
          result.innerHTML = getAlert('success',
            'Logged in with IdentityId: ' + output.identityId + '<br>');
...
function updateActions() {
  ...
  if (identityId == null) {
    // Unauthenticated
    result.innerHTML = getAlert('info',
      '<p>Please login to upload and see your private content.</p>');
    actions.innerHTML =
...
function uploadToS3(params) {
  ...
  if (identityId == null) {
    ...
    s3.putObject(params, function(err, data) {
      result.innerHTML =
        err ? getAlert('danger', 'Error!' + err + err.stack) : getAlert(
          'success', 'Uploaded.');
    });
  }
...
function updateContent() {
  ...
  if (identityId != null) {
    var privateContentIndexKey = 'private/index/' + identityId +
      '/content.json';
...
function getSignedUrlFromKey(key) {
  ...
  console.log("getSignedUrlFromKey: " + key);
...
function renderContent() {
  ...
  index = {};
  console.log("publicContent: " + publicContent.index);
  if (publicContent.index != null) {
    publicContent.index.forEach(function(element) {
      element.isPublic = true;
      element.isOwner = (identityId != null && element.identityId ==
        identityId);
      index[element.objectKey] = element;
    });
  }
  console.log("privateContent: " + privateContent.index);
  ...
  for (var objectKey in index) {
    ...
    console.log("render element: " + JSON.stringify(element));

    html += '<div class="col-sm-3 thumbnail alert ' +
         (element.isPublic ? 'alert-success' : 'alert-warning') + '"">' +
         (element.isOwner ?
           '<button type="button" class="close" onclick=deleteContent("' +
    ...
    html2 += `
    <div class="col-sm-3 thumbnail alert ${element.isPublic ? 'alert-success' : 'alert-warning'}">

      ${element.isOwner ? `<button type="button" class="close" onclick=deleteContent("${objectKey}")>&times;</button>`  : ''}

      <h4 class="text-center">${element.title}</h4>

      <a data-toggle="modal" data-target="#myModalDetail" onclick=showContent("${objectKey}")>
          <img class="img-rounded"
              src="${getSignedUrlFromKey(element.thumbnailKey)}"
              alt="${element.title}">
      </a>
